...
# ---------------- START PATCH: MultiTimeframeReversalFilter (ADD) -----------------
class MultiTimeframeReversalFilter:

    def check_reversal_pressure(self, asset, entry_direction):
        """
        Returns True if SAFE to enter,
        Returns False if higher timeframe is reversing against the trade.
        """

        # Analyze 1M, 5M, 15M via AI trend engine
        try:
            t1 = ai_trend_engine.analyze_timeframe(asset, "fast")
            t5 = ai_trend_engine.analyze_timeframe(asset, "medium")
            t15 = ai_trend_engine.analyze_timeframe(asset, "slow")
        except Exception:
            return True  # fail-safe allow if analysis fails

        d1 = t1.get("direction")
        d5 = t5.get("direction")
        d15 = t15.get("direction")

        # If higher TF disagrees strongly → reversal zone
        if entry_direction == "CALL":
            if d5 == "PUT" and d15 == "PUT":
                return False
        if entry_direction == "PUT":
            if d5 == "CALL" and d15 == "CALL":
                return False

        return True
# ---------------- END PATCH: MultiTimeframeReversalFilter (ADD) -----------------
